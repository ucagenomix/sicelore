...
```{r message=FALSE, warning=FALSE, echo=FALSE, eval=T, fig.height=6, fig.width=18}

DefaultAssay(object = illu1141) <- "ISO"

#pdf("dotplotiso.pdf", width=24, height=8, useDingbats=FALSE)

lst <- c('Clta','Pkm','Myl6','Sox2','Rpl22l1','Evl','Meg3','Dbi','Serpinh1','Vim','Slc1a3','Sh3glb1','Gnas','Gng5','Cd63','Clic1','Csrp1','Mdk','Id4','Dctn2')
isolist <- c()
for(i in 1:length(lst)){
  isolist <- c(isolist, rownames(illu1141@assays$ISO@counts[grep(paste(lst[i],"-",sep=""), rownames(illu1141@assays$ISO@counts)),]))
}
DotPlot(object = illu1141, features = isolist, reduction="tsne", cols = c("grey","blue")) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) # RotatedAxis

lst <- c('Ccnd2','Ppp1r1a','Gnai2','Slc25a5','Tead2','B9d1','Pmm1','Zfand6','Myl12a','Cdk6','Nek6','H2afv','Selm','Shisa4','Mfap2','Ezr','Tubb4b','Mrfap1')
isolist <- c()
for(i in 1:length(lst)){
  isolist <- c(isolist, rownames(illu1141@assays$ISO@counts[grep(paste(lst[i],"-",sep=""), rownames(illu1141@assays$ISO@counts)),]))
}
...
lst <- c('Arl2bp','Rps27l','Atp5g2','Idh2','Nt5dc2','Rmst','Cenpw','Lhx2','Krtcap2','Pgp','Ppib','Aldoa','Rpl29','Tecr','Capns1','H1f0','Tubb2a','Stmn4')
isolist <- c()
for(i in 1:length(lst)){
  isolist <- c(isolist, rownames(illu1141@assays$ISO@counts[grep(paste(lst[i],"-",sep=""), rownames(illu1141@assays$ISO@counts)),]))
}
...
lst <- c('Pcsk1n','Rtn1','Crmp1','Calm2','Ppp2r2b','Dlx2','Nnat','Pfn2','Dlx6os1','Dlx1','Sp9','Tcf4','Rgs2','Dynlt1c','Gadd45g','Pantr1','Ddah2','Nfib','Tmem176b','Insm1','Hnrnpa1','Carhsp1')
isolist <- c()
for(i in 1:length(lst)){
  isolist <- c(isolist, rownames(illu1141@assays$ISO@counts[grep(paste(lst[i],"-",sep=""), rownames(illu1141@assays$ISO@counts)),]))
}
