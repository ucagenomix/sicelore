...
median(s1141@meta.data$nUMIsProfiled)

common.genes <- intersect(rownames(s1141@assays$RNA@data), rownames(s1141@assays$ISOG@data))
dat <- s1141@assays$ISOG@data[common.genes,]
s1141[["ISOGCOMMON"]] <- CreateAssayObject(counts = dat)

s1141@meta.data$nFeaturesProfiled <- s1141@meta.data$nFeature_ISOGCOMMON*100/s1141@meta.data$nFeature_RNA
median(s1141@meta.data$nFeaturesProfiled)
...
ggplot(data=s1141@meta.data, aes(x=nCount_RNA,y=nCount_ISOG)) +  geom_point(shape = 21, colour = "black", fill = col[as.numeric(s1141@meta.data$index)]) +
      theme_bw() + theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank()) +
      ggtitle("Fig.1f - UMIs (r=0.99, r2=0.99)") +
      geom_abline(slope=1, intercept=-15, linetype=3) +
      labs(x="Illumina",y="Nanopore")

x <- cor.test(s1141@meta.data$nFeature_RNA, s1141@meta.data$nFeature_ISOG, method="pearson")$estimate
x*x

ggplot(data=s1141@meta.data, aes(x=nFeature_RNA,y=nFeature_ISOG)) +  geom_point(shape = 21, colour = "black", fill = col[as.numeric(s1141@meta.data$index)]) +
      theme_bw() + theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank()) +
      ggtitle("Fig.1e - Genes (r=0.99, r2=0.97)") +
      geom_abline(slope=1, intercept=-15, linetype=3) +
      labs(x="Illumina",y="Nanopore")

plot1 <- ggplot(data=s1141@meta.data, aes(nCount_RNA)) + 
...
DefaultAssay(object = s1141) <- "ISO"
general <- c('Myl6','Clta','Tecr','Pkm','Cdc42','Smarcb1','Rtn4','Rps24','Bin1','Ergic3','Sh3glb1','Psap','Gnas')
...
DotPlot(object = s1141, features = lst, scale.max = 100) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) # RotatedAxis
...
ggplot(tmp, aes(x=clusters, y=percentage, fill=isoforms)) + 
...
s1141 <- readRDS("output/rna.labels.rds")
DefaultAssay(object = s1141) <- "RNA"
dat <- downsampleMatrix(s1141@assays$RNA@counts, 0.45, bycol=TRUE)
...
mean(s1141@meta.data$nCount_RNA)
median(s1141@meta.data$nCount_RNA)
...
mean(s1141@meta.data$nCount_ISOG)
median(s1141@meta.data$nCount_ISOG)
