...
%age of illumina molecules profiled with Nanopore reads (190 cells)
===================
...
%age of illumina molecules profiled with Nanopore reads (951 cells)
===================
```{r message=FALSE, warning=FALSE, echo=FALSE, eval=T, fig.height=4, fig.width=6}
...
Correlation plots illumina/Nanopore (R2)
===================
```{r message=FALSE, warning=FALSE, echo=FALSE, eval=T, fig.height=4, fig.width=6}
...
Correlation Illumina / Nanopore (Data including 0)
===================
```{r message=FALSE, warning=FALSE, echo=FALSE, eval=T, fig.height=4, fig.width=6}
...
Correlation Illumina / Nanopore (Data excluding 0)
===================
```{r message=FALSE, warning=FALSE, echo=FALSE, eval=T, fig.height=4, fig.width=6}
...
for (i in (1:length(common.cells))){
  t <- data.frame(illu1141@assays$RNA@data[common.genes,common.cells[i]], illu1141@assays$ISOG@data[common.genes,common.cells[i]])
  t2 <- t[which(rowSums(t) > 0),]
  colnames(t2) <- c("illu","nano")
  df <- rbind(df, data.frame("Illu/ONT same cell",cor.test(t2$illu, t2$nano)$estimate))
}
colnames(df) <- c("type","cor")
rownames(df) <- c()
...
for (i in (1:length(common.cells))){
  ...
  for (j in (1:length(random))){
    if(i != random[j]){
      ...
      t2 <- t[which(rowSums(t) > 0),]
      colnames(t2) <- c("illu","nano")
      df2 <- rbind(df2, data.frame("Illu/ONT different cell",cor.test(t2$illu, t2$nano)$estimate))
    }
  }
}
colnames(df2) <- c("type","cor")
