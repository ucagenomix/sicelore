...
public class TagReadWithSequence extends CommandLineProgram
{
  ...
  protected int doWork()
  {
    ...
    try
    {
      ...
      try { localSamReader.close();localSAMFileWriter.close(); } catch (Exception localException2) { System.err.println("can not close stream"); } } finally { try { localSamReader.close();localSAMFileWriter.close(); } catch (Exception localException3) { System.err.println("can not close stream");
      } }
    return 0;
  }
