...
Parses short read data and retrieves info on used cellBCs and UMIs.
...
Adds gene names, read sequence and QV values. Required for cellBC and UMI assignment

[5) CellBC and UMI assignment to Nanopore SAM records](#bc-assignment)
...
Can be omitted for small runs (< 5 million reads). Prior to mapping the fastqs are split into chunks using [fastp](https://github.com/OpenGene/fastp).

Mapping, [tagging with gene names](#nanopore-tagging) and [cellBC/UMI assignment](#bc-assignment) are sequentially done with those batches to allow paralellization.

```console

fastp -i nanopore_reads.fastq -Q -A --thread 1 --split_prefix_digits=4 --out1=sub.fastq --split=12

```

### Parallel minimap2 mapping

command shown is for FASTQ batch "0001.sub.fastq", should be repeated for each batch.

```bash

minimap2 -ax splice -uf --MD -N 100 --sam-hit-only -t 20 --junc-bed junctions.bed <path to minimap2 index file> 0001.sub.fastq > 0001.sub.sam
...
can be generated with **`paftools.js gff2bed -j ann.gtf'** (Paftools is part of the minimap2 distribution)
...
command shown is for BAM batch "0001.sub.bam", should be repeated for each batch.
...
SAMrecords are tagged with read sequence (US tag) and read quality (UQ tag). Is used for cellBC / UMI assignment and molecule consensus calculation.

QVs are currently just used to generate some QV stats during cellBC and UMI assignment -> adding QVs is optional.
...
10x Illumina File parsed by [IlluminaParser.jar](#parsing-illumina-data) (parsedForNanopore_v0.2.obj)
...
### merge chunk bam files

In case of parrallel chunk processing, all Cell barcode and UMI assigned Bam files need to be merged.

Requires: Picard-tools or Samtools

```bash

java -jar -Xmx22g ~/picard-tools-1.119/MergeSamFiles.jar INPUT=0001.sub.GEUS10xAttributes.bam INPUT=0002.sub.GEUS10xAttributes.bam INPUT=... ASSUME_SORTED=true USE_THREADING=true TMP_DIR=/scratch/tmp/ MAX_RECORDS_IN_RAM=100000000 OUTPUT=GEUS10xAttributes.umifound.bam VALIDATION_STRINGENCY=SILENT
...
SAM records matching known genes are grouped by UMI and analyzed for matching Gencode transcripts. SAM records with extensive non-matching sequences at either end, hard- or soft clipping are discarded (MAXCLIP parameter, default = 150 nt ). To assign a UMI to a Gencode transcript a STRICT and a SOFT strategy can be applyed (METHOD parameter). Using ***STRICT*** strategy, a read is assigned to a given transcript when it recapitulates the full exon-exon junction layout authorizing a 2 bases margin (DELTA parameter, default 2) of added or lacking sequences at exon boundaries, to allow for indels at exon junctions and imprecise mapping by Minimap2. For each UMI, all its reads are analyzed and the UMI is assigned to the Gencode transcript supported by the majority of the reads.

If an equal number of reads supports two different Gencode transcript, the UMI is considered as ambiguous and randomly assigned to one of the top scoring isoforms (AMBIGUOUS_ASSIGN=true) / or not assigned to any isoform (default, AMBIGUOUS_ASSIGN=false).
...
.tsv file listing, one per line, the cell barcodes that need to be quantified (e.g. cellBC.951.tsv in github /barcodes/ directory)

**REFFLAT=** (required)

Can be generated base on Gencode GTF file for genome build of interest for mapping with ***gtfToGenePred*** from [UCSC](http://hgdownload.cse.ucsc.edu/admin/exe/linux.x86_64/)
...
SOFT more lenient way of assignation requiring observation of one isoform specific exon-exon junction
...
java -jar -Xmx22g sicelor.jar IsoformMatrix I=GEUS10xAttributes.umifound.bam REFFLAT=refFlat.txt CSV=barcodes.tsv DELTA=2 MAXCLIP=150 METHOD=SOFT AMBIGUOUS_ASSIGN=false PREFIX=out
...
The speed of consensus sequence computation is dependent of the sequencing depth wich induce a low/high number of multi-reads molecules. With a 20 core Xeon node it processes about 150,000 - 300,000 UMIs/hour. For time optimization, this step could be parrallelized, for instance on a per chromosome basis, and dispense on a calcul cluster.
...
java -jar -Xmx44g sicelor.jar ComputeConsensus I=GEUS10xAttributes.umifound.chr1.bam O=molecules.chr1.fa T=20 TMPDIR=/tmp/ POAPATH=/share/apps/local/bio-pipeline/poaV2/ RACONPATH=/share/apps/local/racon/bin/ MINIMAP2PATH=/share/apps/local/minimap2/
