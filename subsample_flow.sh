...
$java -jar -Xmx4g Jar/Sicelore-1.0.jar ComputeConsensus T=10 I=output_dir/GEUS10xAttributes_umifound_.bam O=output_dir/consensus.fa
...
# add cellBCs/UMIs tags to molecules SAM records
...
# tag molecules with gene name and export percentage identity
$java -jar -Xmx22g Jar/Sicelore-1.0.jar AddGeneNameTag I=output_dir/molecule.tags.bam O=output_dir/molecule.tags.GE.bam REFFLAT=Gencode/gencode.v18.mm10.refFlat.txt GENETAG=GE ALLOW_MULTI_GENE_READS=true USE_STRAND_INFO=true VALIDATION_STRINGENCY=SILENT
samtools index output_dir/molecule.tags.GE.bam
$java -jar -Xmx4g Jar/Sicelore-1.0.jar ExportMoleculeMetrics I=output_dir/molecule.tags.bam O=output_dir/sicelore_molecules_metrics.txt GENETAG=GE
	
# generate molecule isoform matrix
$java -jar -Xmx4g Jar/Sicelore-1.0.jar IsoformMatrix DELTA=2 METHOD=STRICT I=output_dir/molecule.tags.GE.bam REFFLAT=Gencode/gencode.v18.mm10.refFlat.txt GENETAG=GE CSV=Barcodes/cellBC.190.tsv OUTDIR=output_dir PREFIX=sicelore.molecule VALIDATION_STRINGENCY=SILENT
